...
class Session:
	...
	def fromConfig(config:InferenceConfig) -> 'Session':
		if config.session_type == "onnx":
			...
			elif config.model_type == 'internvl-pact':
				return AclInternVLPACTSession(config)
			else:
...
class AclQwenVLPACTSession(Session):
	...
	def run(self, input_ids:np.ndarray, image_mask, pixel_values=None,):
		...
		if pixel_values is not None:
			...
			image_len = np.sum(image_mask)
			self.vision_model.unload()
			logging.info(f"vision_model inference time: {time.time() - start_time:.2f}s")
		else:
			image_start_pos = -1
			image_len = 0
		...
		if use_pact:
			...
			pruned_hidden_states, pruned_position_ids, self.weights, reduction= pact(image_hidden_states, query_states_before_rope, key_states_before_rope, key_states, None, image_mask[:, -seq_len:], real_position_ids, self.pact_config_path, 'qwenvl-pact')
...
class AclInternVLSession(Session):
	context = None
	def __init__(self,config:InferenceConfig)->None:
		super().__init__(config)
		self.context = initResource(config.device)
		...
		if config.visual_path is not None:
			self.vision_model = ACLModel(config.vision_model,context=self.context,mode=config.acl_mode)
			self.mlp_model = ACLModel(config.mlp_model,context=self.context,mode=self.acl_mode)
		self.llm_model = ACLModel(config.llm_model,context=self.context,mode=self.acl_mode)
		# input: pixel_values:1,900,1176
		self.embedding_model = ACLModel(config.embedding_model,context=self.context,mode=config.acl_mode)
		# input: input_ids:1,1
	...
	def run(self, input_ids:np.ndarray,  image_mask, pixel_values=None,):
		...
		while l < seq_len:
			r = min(seq_len,r)
			self.input_ids[:,:r-l] = input_ids[:,l:r]
			cache,mask,pos_ids = self.kvCache.getInputs(self.max_len, dim4 = False)
			if l < image_start_pos:
				self.input_embeds[:,:r-l,:] = self.embedding_model.inference([self.input_ids])[0]
			elif l >= image_start_pos + image_len:
				self.input_embeds[:,:r-l,:] = self.embedding_model.inference([self.input_ids])[0]
...
class AclInternVLPACTSession(Session):
	context = None
	def __init__(self,config:InferenceConfig)->None:
		super().__init__(config)
		self.context = initResource(config.device)
		...
		self.embedding_model = ACLModel(config.embedding_model,context=self.context,mode=config.acl_mode)	
		llm_model_1 = ACLModel(config.llm_model.replace('internvl_language_model.om','pact/internvl_llm_1_6_mask.om'),context=self.context,mode=self.acl_mode)
		llm_model_2 = ACLModel(config.llm_model.replace('internvl_language_model.om','pact/internvl_llm_7_24_mask.om'),context=self.context,mode=self.acl_mode)
		self.llm_model = [llm_model_1, llm_model_2]
	...
	def run(self, input_ids:np.ndarray, image_mask, pixel_values=None,):
		...
		while l < seq_len:
			r = min(seq_len,r)
			self.input_ids[:,:r-l] = input_ids[:,l:r]
			cache,mask,pos_ids = self.kvCache[0].getInputs(self.max_len, dim4 = True)
			if l < image_start_pos:
				self.input_embeds[:,:r-l,:] = self.embedding_model.inference([self.input_ids])[0]
			elif l >= image_start_pos + image_len:
				self.input_embeds[:,:r-l,:] = self.embedding_model.inference([self.input_ids])[0]
		...
		if use_pact:
			...
			key_states = np.concatenate(self.key_states_list, axis=2)
			image_hidden_states = np.concatenate(self.hidden_states_list, axis=1)
			real_position_ids = np.concatenate(self.real_position_ids, axis=1)

			pruned_hidden_states, pruned_position_ids, self.weights, reduction= pact(image_hidden_states, query_states_before_rope, key_states_before_rope, key_states, None, image_mask[:, -seq_len:], real_position_ids, self.pact_config_path, 'internvl-pact')
			logging.info(f"PACT 保留率: {1.0*reduction[1]/reduction[0]:.2f}%")
			seq_len = pruned_hidden_states.shape[1]
		...
		while l < seq_len:
			r = min(seq_len,r)
			cache,mask,pos_ids = self.kvCache[1].getInputs(self.max_len, dim4 = True)
