...
class Qwen2VLRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        shape_3 = 64
        inv_freq_expanded = self.inv_freq[None, None, :, None].float().expand(3, position_ids.shape[1], shape_3, 1)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            weight_3d = self.proj.weight.clone()
            weight_2d = weight_3d.view(
                self.embed_dim,
            ...
            hidden_states = F.conv2d(hidden_states.to(dtype=target_dtype), weight_2d, None).view(-1, self.embed_dim)
...
class Qwen2VLAttention(nn.Module):
    ...
    def forward(
        ...
        if use_cache:
            new_key_value = torch.cat([key_states.unsqueeze(0), value_states.unsqueeze(0)], dim=0)
        ...
        return attn_output, attn_weights, past_key_value, new_key_value
...
class Qwen2VLSdpaAttention(Qwen2VLAttention):
    ...
    def forward(
        ...
        key_head_num = 2
        value_head_num = 2
        query_states = query_states.view(bsz, q_len, query_head_num, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, key_head_num, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, value_head_num, self.head_dim).transpose(1, 2)
        ...
        return attn_output, None, past_key_value, new_key_value, query_states_before_rope, key_states_before_rope
...
class Qwen2VLDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2VLConfig, layer_idx: int):
        ...
        self.self_attn = QWEN2_VL_ATTENTION_CLASSES["sdpa"](config, layer_idx)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Self Attention
        hidden_states, self_attn_weights, present_key_value, new_key_value, qurey_states_before_rope, key_states_before_rope = self.self_attn(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_value=past_key_value,
        ...
        if PACT and Layer1_4:
            outputs += (qurey_states_before_rope, key_states_before_rope)
...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw=None) -> torch.Tensor:
        grid_thw = torch.tensor([[1, 30, 30]], dtype=torch.int64)
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if use_cache:
                ...
                new_key_value = layer_outputs[3 if output_attentions else 2]
                new_key_values.append(new_key_value.unsqueeze(0))

            if output_attentions:
                all_self_attns += (layer_outputs[1],)

            if i == 3 and PACT and Layer1_4:
                qurey_states_before_rope = layer_outputs[4 if output_attentions else 3]
                key_states_before_rope = layer_outputs[5 if output_attentions else 4]

                new_key_values = torch.cat(new_key_values, dim=0)
                return hidden_states, new_key_values, qurey_states_before_rope, key_states_before_rope
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        print("shape of causal_mask", causal_mask.shape)
        print(causal_mask)
        return causal_mask
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if PACT and Layer1_4:
            ...
            return hidden_states, new_key_values, query_states_before_rope, key_states_before_rope
